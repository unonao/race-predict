...
def savefile(year):
    return RACR_URL_DIR + "/" + str(year) + ".txt"
...
def get_race_url():
    ...
    for year in range(2008, now_datetime.year):
        for month in range(1, 13):
            race_url_file = savefile(year)  # 保存先ファイル
            if not os.path.isfile(race_url_file):  # まだ取得していなければ取得
                logger.info("getting urls ("+str(year) + ")")
                get_race_url_by_year_and_mon(driver, year)
    # 今月分は毎回取得
    logger.info("getting urls ("+str(now_datetime.year) + ")")
    get_race_url_by_year_and_mon(driver, now_datetime.year)

    driver.close()
    driver.quit()


def get_race_url_by_year_and_mon(driver, year):
    race_url_file = savefile(year)  # 保存先ファイル
    ...
    # ダートを選択
    terms = driver.find_element_by_id("check_track_2")
    terms.click()
    # 東京競馬場をチェック
    terms = driver.find_element_by_id("check_Jyo_05")
    terms.click()
        ...
        with open(race_url_file, mode='w') as f:
            ...
            while True:
                ...
                try:
                    ...
                    break
        logging.info("got " + str(total) + " urls of " + str(total_num) + " ("+str(year) + ")")
    else:
        logging.info("already have " + str(pre_url_num) + " urls ("+str(year) + ")")
